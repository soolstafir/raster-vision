...
def make_neg_chips(image_dataset, boxes, classes, chip_dir, num_neg_chips,
                   config):
    box_db = BoxDB(boxes)
...
def _make_train_chips(image_path, annotations_path, chip_dir, chip_label_path,
    ...
    boxes, classes, _ = get_boxes_from_geojson(
        chip_label_path, image_dataset, label_map=label_map)
    ...
    num_neg_chips = int(num_pos_chips * config.neg_ratio)
    make_neg_chips(
        image_dataset, boxes, classes, chip_dir, num_neg_chips, config)
